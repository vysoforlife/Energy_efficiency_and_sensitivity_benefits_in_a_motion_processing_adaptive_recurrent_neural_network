...
def calculate_category_response_over_frames_xy(category_indices, models1, models2, test_data, freeze_frame, output_dim):
    frame_responses1 = []
    frame_errors1 = []
    frame_responses2 = []
    frame_errors2 = []
    ...
    for frame in range(num_of_frames):
        ...
        for model1, model2 in zip(models1, models2):
            ...
            with torch.no_grad():
                predictions1, _, _, _ = model1(frozen_sequences)
                predictions2, _, _, _, _ = model2(frozen_sequences)
                responses1 = predictions1[:, frame, output_dim]
                responses2 = predictions2[:, frame, output_dim]
...
while True:
    try:
        dimension = input("Choose dimension to analyze (x/y/both) or 'q' to quit: ").lower()
        if dimension == 'q':
            break
        ...
        category = int(input("Enter a category number (1-20) to analyze: "))
        if not (1 <= category <= 20):
        ...
        if dimension in ['x', 'both']:
            category_indices_x = categories_x[category - 1]
            if len(category_indices_x) == 0:
                print("No sequences in this X category.")
            else:
                responses_x = calculate_category_response_over_frames_xy(
                    category_indices_x, models1, models2, test_data, freeze_frame, 0)
                responses.append(responses_x)

        if dimension in ['y', 'both']:
            category_indices_y = categories_y[category - 1]
            if len(category_indices_y) == 0:
                print("No sequences in this Y category.")
            else:
                responses_y = calculate_category_response_over_frames_xy(
                    category_indices_y, models1, models2, test_data, freeze_frame, 1)
                responses.append(responses_y)
        ...
        if dimension == 'both':
            # Average the responses from x and y
            frame_responses1 = np.mean([resp[0] for resp in responses], axis=0)
            frame_errors1 = np.mean([resp[1] for resp in responses], axis=0)
            frame_responses2 = np.mean([resp[2] for resp in responses], axis=0)
            frame_errors2 = np.mean([resp[3] for resp in responses], axis=0)
        else:
            frame_responses1, frame_errors1, frame_responses2, frame_errors2 = responses[0]
        ...
        plt.tight_layout()
        plt.savefig(f'waterfall_{dimension}_category_{category}_freeze_{freeze_frame}.svg', format='svg')
        plt.show()
